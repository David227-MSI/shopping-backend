...
public class ProductMediaService {
    ...
    public List<ProductMedia> uploadBatch(Integer productId, List<MultipartFile> files) throws IOException {
        ...
        for (int i = 0; i < files.size(); i++) {
            ...
            // 0. 檔案大小驗證
            if (file.getSize() > MAX_SIZE) {
                log.warn("File size exceeds limit for product ID {} during batch upload. File: {}, Size: {}", productId, file.getOriginalFilename(), file.getSize());
            ...
            try {
                ...
                newlyUploadedMedia.add(savedMedia); // 將新保存的媒體添加到新上傳列表中
                 ...
                 log.error("Unexpected error during saving media record for product ID {} during batch upload.", productId, e);
        ...
        if (!newlyUploadedMedia.isEmpty()) {
            ...
            if (existingMain.isEmpty()) {
                log.info("No existing main image found for product ID {}. Setting the first newly uploaded media as main.", productId);
                ...
                try {
                    ...
                    log.error("Failed to save updated media record (set as main) to DB for ID {}", firstUploaded.getId(), e);
                    // 這裡的失敗比較關鍵，可能需要向前端報告
                } catch (Exception e) {
                     log.error("Unexpected error saving updated media record (set as main) to DB for ID {}", firstUploaded.getId(), e);
                }
            } else {
                 log.info("Existing main image found for product ID {}. No new main image set automatically.", productId);
                 // 如果有主圖，不需要自動設定新的主圖
                 // 但可能需要重新排序，確保新上傳的圖片有正確的 order
                 // 由於 uploadBatch 已經按順序賦予 order，這裡通常不需要額外觸發 reorder，除非你需要將新上傳的圖片插入到特定位置
                 // 如果需要確保所有圖片（包括舊的和新的）的 order 在批量上傳後是連續且主圖排第一，可以呼叫 reorderMediaAfterMainSet(productId, existingMain.get().getId());
        ...
        log.info("Finished batch uploading media for product ID {}. Successfully saved {} records.", productId, savedMediaList.size());
    ...
    private void reorderMediaAfterMainSet(Integer productId, Integer mainMediaId) {
         ...
         // 將主圖移到列表最前面
         mediaList.sort((a, b) -> {
             if (a.getId().equals(mainMediaId)) return -1; // 主圖排第一
             if (b.getId().equals(mainMediaId)) return 1;
         ...
         // 保存重新排序的結果
         try {
              log.info("Saving reordered media for product ID {} after main set. Count: {}", productId, mediaList.size());
             repo.saveAll(mediaList);
              log.info("Successfully saved reordered media for product ID {}.", productId);
         } catch (DataAccessException e) {
