...
public class ProductMediaController {
    ...
    /* ---------- 2. 上傳單一媒體 (如果需要，但推薦使用批量上傳) ---------- */
    // @PostMapping(value = "/product/{pid}/upload", consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
    // public ResponseEntity<ApiRes<ProductMediaDto>> upload(
    //         @PathVariable Integer pid,
    //         @RequestPart MultipartFile file,
    //         @RequestParam(defaultValue = "false") boolean main,
    //         @RequestParam(required = false) Integer order) throws IOException {
    //     log.info("Received POST request to upload single media for product ID: {}", pid);
    //     ProductMedia saved = mediaSvc.upload(pid, file, main, order);
    //     log.info("Single media uploaded successfully. ID: {}", saved.getId());
    ...
    public ResponseEntity<ApiRes<List<ProductMediaDto>>> uploadBatch(
        ...
        try {
            ...
            return ResponseEntity.status(HttpStatus.CREATED).body(ResponseFactory.success(savedMediaDtos));
             ...
             return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(ResponseFactory.fail("批量上傳媒體時發生未知錯誤"));
        }
    }
